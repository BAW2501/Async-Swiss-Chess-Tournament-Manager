...
tournament_data = {
    ...
    'pairings': [],
    'last_update': None,
    'data_version': 0  # Track data version for client sync
}
...
def render_header(last_update):
    ...
    <div class="header">
        ...
        <div class="last-update"><i class="far fa-clock"></i> <span id="last-update-time">{last_update if last_update else 'Loading...'}</span></div>
    </div>
    """
...
def render_nav(active_page):
    ...
    <div class="nav-buttons">
        <a href="javascript:void(0)" class="nav-button {'active' if active_page == 'combined' else ''}" onclick="navigateToView('combined')">
            <i class="fas fa-columns"></i>Combined
        </a>
        <a href="javascript:void(0)" class="nav-button {'active' if active_page == 'standings' else ''}" onclick="navigateToView('standings')">
            <i class="fas fa-ranking-star"></i>Standings
        </a>
        <a href="javascript:void(0)" class="nav-button {'active' if active_page == 'pairings' else ''}" onclick="navigateToView('pairings')">
...
def render_standings_table(standings):
    ...
    html += '<th>DE</th>'
    html += '<th>Ber</th>'
    html += '</tr></thead><tbody id="standings-tbody">'
    ...
    for player in standings:
        ...
        elif player.get('Pos') == 2:
            rank_class = 'rank-2'
            icon = '<i class="fas fa-medal"></i> '
        elif player.get('Pos') == 3:
            rank_class = 'rank-3'
            icon = '<i class="fas fa-award"></i> '
        
        html += f'<tr><td class="rank {rank_class}">{icon}{player.get("Pos", "")}</td>'
        html += f'<td class="player-cell">{player.get("Player Name", "")}</td>'
        html += f'<td class="points">{player.get("Pt", "")}</td>'
        html += f'<td>{player.get("BucT", "")}</td>'
        html += f'<td>{player.get("DE", "N/A")}</td>'
        html += f'<td>{player.get("Ber", "N/A")}</td></tr>'
...
def render_pairings_table(pairings, show_filter=True):
    ...
    if show_filter:
        ...
        <div class="filter-container">
            <button class="filter-button" data-filter="all" onclick="setFilter('all')">All</button>
            <button class="filter-button" data-filter="pending" onclick="setFilter('pending')">Live</button>
            <button class="filter-button" data-filter="finished" onclick="setFilter('finished')">Done</button>
        </div>
        """
    ...
    for pairing in pairings:
        status = pairing.get('result_status', 'Pending')
        html += f'<tr class="pairing-row" data-status="{status}">'
        html += f'<td><span class="round-badge">{pairing.get("Round", "")}</span></td>'
        html += f'<td><span class="board-number">{pairing.get("Board", "")}</span></td>'
        html += f'<td class="player-cell">{pairing.get("White Name", "")}</td>'
        html += '<td class="vs-separator" style="font-size:0.8em; color:#ccc;">vs</td>'
        html += f'<td class="player-cell">{pairing.get("Black Name", "")}</td>'
...
# Auto-refresh JavaScript with state management
AUTO_REFRESH_SCRIPT = """
<script>
// State management
...
function initializeFromURL() {
    ...
    if (filter && (filter === 'all' || filter === 'pending' || filter === 'finished')) {
...
// Navigate to a view while preserving filter
function navigateToView(view) {
    let url = '/';
    if (view === 'standings') url = '/standings';
    if (view === 'pairings') url = '/pairings';
...
function updateStandings(standings) {
    ...
    standings.forEach(player => {
        ...
        const de = player.DE !== undefined && player.DE !== null ? player.DE : 'N/A';
        const ber = player.Ber !== undefined && player.Ber !== null ? player.Ber : 'N/A';
        
        html += `<tr><td class="rank ${rankClass}">${icon}${player.Pos}</td>`;
        html += `<td class="player-cell">${player["Player Name"]}</td>`;
        html += `<td class="points">${player.Pt}</td>`;
        html += `<td>${player.BucT}</td>`;
        html += `<td>${de}</td>`;
        html += `<td>${ber}</td></tr>`;
    });
...
function updatePairings(pairings) {
    ...
    if (!tbody || !pairings || pairings.length === 0) return;
    
    let html = '';
    pairings.forEach(pairing => {
        const status = pairing.result_status || 'Pending';
        html += `<tr class="pairing-row" data-status="${status}">`;
        html += `<td><span class="round-badge">${pairing.Round}</span></td>`;
        html += `<td><span class="board-number">${pairing.Board}</span></td>`;
        html += `<td class="player-cell">${pairing["White Name"]}</td>`;
        html += '<td class="vs-separator" style="font-size:0.8em; color:#ccc;">vs</td>';
        html += `<td class="player-cell">${pairing["Black Name"]}</td>`;
        
        let resultHtml = '<span class="result result-pending">Live</span>';
        if (status === 'White Win') {
            resultHtml = '<span class="result result-win">1-0</span>';
        } else if (status === 'Black Win') {
            resultHtml = '<span class="result result-loss">0-1</span>';
        } else if (status === 'Draw') {
            resultHtml = '<span class="result result-draw">½-½</span>';
        }
...
async function refreshData() {
    ...
    try {
        ...
        // Only update if data version changed
        if (data.data_version !== lastDataVersion) {
            ...
            // Update tables
            if (data.standings) updateStandings(data.standings);
            if (data.pairings) updatePairings(data.pairings);
            
            console.log(`✅ Data updated (version ${data.data_version})`);
        }
...
document.addEventListener('DOMContentLoaded', function() {
    initializeFromURL();
    console.log('✅ Auto-refresh enabled - updates every 5 seconds');
    console.log('📍 Current view:', currentView, '| Current filter:', currentFilter);
});
...
class ExcelFileHandler(FileSystemEventHandler):
    ...
    def on_modified(self, event):
        if event.src_path.endswith('tournament.xlsx') or event.src_path.endswith('.xlsx'):
            current_time = time.time()
            if current_time - self.last_modified > 2:
                self.last_modified = current_time
                print(f"[{datetime.now().strftime('%H:%M:%S')}] Excel file changed - reloading data...")
                time.sleep(1.5)  # Longer wait for Excel to finish saving
                load_tournament_data()

def load_tournament_data():
    """Load tournament data with retry logic and proper error handling"""
    max_retries = 3
    retry_delay = 1.0
        ...
        for attempt in range(max_retries):
            try:
                ...
                try:
                    ...
                    print(f"Could not load standings (attempt {attempt + 1}): {e}")
                    if attempt < max_retries - 1:
                        time.sleep(retry_delay)
                        continue
                    tournament_data['standings'] = []
                
                # Try to load pairings
                try:
                    pairings_df = pd.read_excel(filepath, sheet_name="Pairings", engine='openpyxl')
                    pairings_list = []
                    for _, row in pairings_df.iterrows():
                        pairing = row.to_dict()
                        white_result = row.get('Results White')
                        black_result = row.get('Results Black')
                        
                        if pd.notna(white_result) and pd.notna(black_result):
                            if white_result == 1:
                    ...
                    print(f"Could not load pairings (attempt {attempt + 1}): {e}")
                    if attempt < max_retries - 1:
                        time.sleep(retry_delay)
                        continue
                ...
                # Update metadata
                tournament_data['last_update'] = datetime.now().strftime('%H:%M:%S')
                tournament_data['data_version'] += 1
                
                print(f"[{datetime.now().strftime('%H:%M:%S')}] ✅ Data loaded successfully (v{tournament_data['data_version']}): "
                      f"{len(tournament_data['standings'])} standings, {len(tournament_data['pairings'])} pairings")
                return
                ...
                if attempt < max_retries - 1:
                    ...
                    print("Failed to load data: file is locked by another process (Excel?)")
            except Exception as e:
                print(f"Error loading tournament data (attempt {attempt + 1}): {e}")
                if attempt < max_retries - 1:
                    time.sleep(retry_delay)
                else:
...
@app.route('/')
def combined_view():
    script = AUTO_REFRESH_SCRIPT.replace('{{ current_view }}', 'combined')
    return render_template_string(
        ...
        auto_refresh_script=Markup(script)
    )

@app.route('/standings')
def standings_view():
    script = AUTO_REFRESH_SCRIPT.replace('{{ current_view }}', 'standings')
    return render_template_string(
        SINGLE_PAGE,
        title='Standings',
        styles=Markup(SHARED_STYLES),
...
@app.route('/pairings')
def pairings_view():
    script = AUTO_REFRESH_SCRIPT.replace('{{ current_view }}', 'pairings')
    return render_template_string(
        SINGLE_PAGE,
        title='Pairings',
        styles=Markup(SHARED_STYLES),
...
def main():
    ...
    print()
    print("    http://localhost:5000           - Combined View")
    ...
    print("   • State preservation (view + filter persisted)")
    print("   • Excel file monitoring with retry logic")
    print("   • F5 refresh keeps your current state")
    print()
